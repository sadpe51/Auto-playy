...
local KEYBIND_AUTO = Enum.KeyCode.C
...
main.Size = UDim2.new(0, 220, 0, 490)
-- ✅ GUI abre no lado direito da tela
...
local function updateDrag(input)
    if dragStart and startPos then
        ...
        local newX = math.clamp(startPos.X.Offset + delta.X, 0, workspace.CurrentCamera.ViewportSize.X - main.AbsoluteSize.X)
        local newY = math.clamp(startPos.Y.Offset + delta.Y, 0, workspace.CurrentCamera.ViewportSize.Y - main.AbsoluteSize.Y)
...
local autoPlayBtn = MakeBtn("AUTO-PLAY: OFF", 231, 196, 12)
local speedBtn    = MakeBtn("SPEED AUTO: OFF", 279, 196, 12)

MakeSep(329)

MakeSectionLabel("AUTO GRAB:", 336)
local grabBtn = MakeBtn("AUTO GRAB: OFF", 351, 196, 12)
...
do
    ...
    colorBar.Position = UDim2.new(0.5, -90, 0, 462)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == KEYBIND_AUTO then
        if moving then stopAll() else startAutoPlay() end
    end
end)
